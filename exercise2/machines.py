...
class Puhti(Supercomputer):
    """
    Puhti at CSC in Finland
    - Overview: https://docs.csc.fi/computing/overview/
    - Partitions: https://docs.csc.fi/computing/running/batch-job-partitions/
    - Batch scripts: https://docs.csc.fi/computing/running/example-job-scripts/
    """
    ...
    def write_job_header(self,job):
        ...
        c+='#SBATCH --job-name={0}\n'.format(job.name)
        c+='#SBATCH --account={0}\n'.format(job.account)
        c+='#SBATCH --partition={0}\n'.format(job.queue)
        c+='#SBATCH --time={0}\n'.format(job.sbatch_walltime())
        c+='#SBATCH --nodes={0}\n'.format(job.nodes)
        c+='#SBATCH --ntasks-per-node={0}\n'.format(job.processes_per_node)
        c+='#SBATCH --cpus-per-task={0}\n'.format(job.threads)
        #c+='#SBATCH --ntasks={0}\n'.format(job.processes)
        c+='#SBATCH --mem-per-cpu={0}\n'.format(job.mem_per_cpu)
        c+='#SBATCH -o {0}\n'.format(job.outfile)
        c+='#SBATCH -e {0}\n'.format(job.errfile)
        return c
    #end def write_job_header
...
Puhti(         532,   2,    20,  382,  100,   'srun',   'sbatch',  'squeue', 'scancel')
